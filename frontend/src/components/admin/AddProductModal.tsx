...
const AddProductModal: React.FC<AddProductModalProps> = ({ isOpen, onClose, onAdd }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      // Dans une application réelle, vous utiliseriez FormData pour uploader les images
      // et obtiendriez les URLs des images depuis la réponse du serveur
      // Pour simplifier, simulons le comportement
      const uploadedImageUrls = await Promise.all(
        imageFiles.map(async (file) => {
          // Simulation d'un upload - dans un cas réel, vous enverriez le fichier au serveur
          // et recevriez l'URL en retour
          return URL.createObjectURL(file); // Ceci est temporaire et juste pour la démo
        })
      );

      const productData = {
        ...formData,
        images: uploadedImageUrls.length > 0 ? uploadedImageUrls : ['https://placehold.co/600x400?text=Image+placeholder']
      };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      // Créer des previews pour les images sélectionnées
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      // Créer des previews pour les images déposées
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="col-span-1 md:col-span-2">
              ...
              >
                ...
                {/* Input pour sélectionner des fichiers */}
                <input 
                  type="file" 
                  ref={fileInputRef}
